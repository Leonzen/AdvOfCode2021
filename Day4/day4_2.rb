...
def calcWinningBlocks(blocks)
    wins = 0
    blocks.each do |block|
        wins += 1 if block.getWinningState()
    end
    wins
end
...
drawnNumbers.each do |number|
    ...
    blocks.each do |block|
        ...
        if (calcWinningBlocks(blocks) == blocks.size())
            lastBlock = block
            break
        end
    end
    break if lastBlock != nil
end


puts lastNumber * lastBlock.sumUmarkedNumbers()