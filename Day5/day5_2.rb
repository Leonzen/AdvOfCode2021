...
class HeatSpot
    ...
    def determineDirection()
        ...
        case
            when c.x != 0 && c.y != 0 then self.direction = self.determineDiagDirection(c) ; self.steps = c.x.abs
    ...
    def determineDiagDirection(c)
        case
            when c.x > 0 && c.y < 0 then self.direction = "lu"
            when c.x < 0 && c.y < 0 then self.direction = "ru"
            when c.x > 0 && c.y > 0 then self.direction = "ld"
            when c.x < 0 && c.y > 0 then self.direction = "rd"
        end
    end
end
...
def markSpots(heatSpot, map)
    map[heatSpot.from.x][heatSpot.from.y] += 1
    for i in 1..heatSpot.steps do
        case
            ...
            when heatSpot.direction == "ld" then map[heatSpot.from.x - i][heatSpot.from.y - i] += 1
            when heatSpot.direction == "lu" then map[heatSpot.from.x - i][heatSpot.from.y + i] += 1
            when heatSpot.direction == "rd" then map[heatSpot.from.x + i][heatSpot.from.y - i] += 1
            when heatSpot.direction == "ru" then map[heatSpot.from.x + i][heatSpot.from.y + i] += 1
        end
    end
end
